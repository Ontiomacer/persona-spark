    ...
    <div className="p-4 space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{persona.role} at {persona.company}</p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="glass-card p-4 space-y-3">
        <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Persona Snapshot</h4>
      ...
      <div className="glass-card p-4">
        <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Interests</h4>
        <div className="flex flex-wrap gap-1.5">
          {persona.interests.map((interest) => (
            <span
              key={interest}
              className="px-2 py-1 rounded-full bg-muted text-foreground text-[10px] border border-border flex items-center gap-1"
            >
      ...
      <div className="glass-card p-4 space-y-3">
        ...
        <div className="space-y-2">
          {[
            { label: "Reply Likelihood", value: persona.responseLikelihood },
            { label: "Personalization Score", value: scores[0] || 75 },
          ].map((metric) => (
            <div key={metric.label} className="flex items-center justify-between gap-3 text-xs">
              ...
              <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-foreground/30 rounded-full" style={{ width: `${metric.value}%` }} />
              </div>
              <span className="text-muted-foreground w-8 text-right font-mono">{metric.value}%</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {subjects.length > 0 && (
        <div className="glass-card p-4">
          <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Subject Lines</h4>
          <div className="space-y-1.5">
            {subjects.map((s, i) => (
              <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 text-xs text-foreground">
